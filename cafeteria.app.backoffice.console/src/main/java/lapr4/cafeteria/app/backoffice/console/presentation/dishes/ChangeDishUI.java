...
import lapr4.cafeteria.dish_management.application.ChangeDishController;
import lapr4.cafeteria.dish_management.domain.Dish;

@SuppressWarnings("squid:S106")
public class ChangeDishUI extends AbstractUI {

    private final ChangeDishController controller = new ChangeDishController();
    ...
    protected boolean doShow() {
        ...
        if (!dishes.iterator().hasNext()) {
            ...
            if (selectedDish != null) {
                try {
                    final boolean changePrice = Console.readBoolean("Do you want to change the price? (y/n)");
                    if (changePrice) {
                        final double newPrice = Console.readDouble("New Price: ");
                        selectedDish = controller.changeDishPrice(selectedDish, newPrice);
                    }
                    final boolean changeNutricionalInfo = Console.readBoolean("Do you want to change the nutricional info? (y/n)");
                    if (changeNutricionalInfo) {
                        final int newCalories = Console.readInteger("New Calories: ");
                        final int newSalt = Console.readInteger("New Salt: ");
                        selectedDish = controller.changeDishNutricionalInfo(selectedDish, newCalories, newSalt);
                    }
                    showDishResult(selectedDish);
